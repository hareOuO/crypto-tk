...
# -DCMAKE_PREFIX_PATH=/opt/og/fides_relic/relic
third_party_build:
	cd $(THIRD_PARTY_DIR) && mkdir -p $(THIRD_PARTY_BUILD_DIR)
	cd $(THIRD_PARTY_BUILD_DIR) && cmake .. -DCMAKE_C_COMPILER=/usr/bin/gcc -DCMAKE_CXX_COMPILER=/usr/bin/g++ \
		-Drelic_DIR=/opt/og/fides_relic \
		-DCMAKE_PREFIX_PATH=/opt/og/fides_relic 
	$(MAKE) -C $(THIRD_PARTY_BUILD_DIR)
